"""Tests for engine API: unified ingest, query, engine management, pipeline control."""
...
def app(db):
    ...
    test_app.state.manifest_registry = ManifestRegistry()
...
class TestUnifiedIngest:
    async def test_unknown_source_returns_404(self, client):
        resp = await client.post("/ingest/nonexistent", json={
...
class TestPipelineControl:
    ...
    async def test_paused_state_persists(self, client):
