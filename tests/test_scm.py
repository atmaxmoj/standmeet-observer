...
Tests repository CRUD and async DB endpoints.
...
class TestScmTaskRepository:
    ...
    def test_multiple_tasks_per_project(self, sync_session):
        ...
        yt_tasks = [t for t in tasks if t["project"] == "YouTeacher"]
        assert len(yt_tasks) == 2

    def test_update_task_status(self, sync_session):
        created = write_scm_task(
            sync_session, "FlexDriver", "Fix CocoaPods", "open", "[]", "run001",
        )
        sync_session.commit()

        updated = update_scm_task(sync_session, created["id"], "done",
                                  "Resolved in episode #5700")
        sync_session.commit()
        assert updated["status"] == "done"

        tasks = get_scm_tasks(sync_session)
        notes = json.loads(tasks[0]["notes"])
        assert "Resolved" in notes[0]
    ...
    def test_filter_by_status(self, sync_session):
        ...
        open_tasks = get_scm_tasks(sync_session, status="open")
        assert len(open_tasks) == 1
        assert open_tasks[0]["title"] == "task1"

        all_tasks = get_scm_tasks(sync_session)
...
class TestScmAsyncDb:
    ...
    @pytest.mark.asyncio
    async def test_count_scm_tasks(self, db, sync_session):
        write_scm_task(sync_session, "A", "t1", "open", "[]", "r1")
        write_scm_task(sync_session, "B", "t2", "done", "[]", "r1")
        sync_session.commit()

        assert await db.count_scm_tasks() == 2
        assert await db.count_scm_tasks(status="open") == 1
