"""Audio capture daemon: record → chunk → transcribe → push to engine."""
...
def run(client: EngineClient, recorders: list[AudioRecorder], keep_chunks: bool = False):
    """
    Main daemon loop:
    1. Recorders run in background, producing WAV chunks every N minutes
    2. When a chunk is ready, transcribe it with whisper
    3. Push transcription to engine via HTTP
