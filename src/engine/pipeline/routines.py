"""Routine composition — agentic multi-turn via MCP tools."""

import asyncio
import logging

from engine.config import Settings
from engine.storage.db import DB

logger = logging.getLogger(__name__)


async def daily_routines(settings: Settings, db: DB) -> int:
    """Run daily routine composition: episodes + playbooks → routines via agentic MCP.

    Returns number of routines created/updated.
    """
    logger.info("starting routine extraction")
    episodes = await db.get_recent_episodes(days=1)
    if not episodes:
        logger.info("no episodes today, skipping routine extraction")
        return 0

    from engine.agents.service import AgentService
    from engine.agents.tools.compose_mcp import create_compose_mcp_server
    from engine.prompts.routine import ROUTINE_PROMPT
    from engine.storage.engine import get_sync_session_factory
    import os

    sync_url = os.environ.get("DATABASE_URL_SYNC", "")
    if not sync_url:
        logger.error("DATABASE_URL_SYNC not configured")
        return 0

    session = get_sync_session_factory(sync_url)()
    try:
        agent = AgentService(settings)
        mcp_server = create_compose_mcp_server(session)
        await asyncio.to_thread(
            agent.run_with_mcp,
            ROUTINE_PROMPT, mcp_server, "compose", "compose_agentic", session,
        )
        session.commit()
    finally:
        session.close()

    count = len(await db.get_all_routines())
    logger.info("Routine extraction: %d routines from %d episodes", count, len(episodes))
    return count
